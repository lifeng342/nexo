...
| 1.3 | 2026-03-04 | **修复广播自投递**：onRemoteEnvelope 新增 SourceInstance 跳过检查，防止发送者实例通过广播重复投递本地用户；**远端信封异步处理**：新增 remoteEnvChan + remoteEnvLoop worker pool，PubSub handler 仅写入 channel 不阻塞接收管道；**移除未使用参数**：dispatchCrossInstance 去掉 localDelivered 参数，pushToLocalClients 简化为无返回值；**配置重构**：route_ttl_seconds 和 instance_alive_ttl_seconds 从 hybrid 移至 cross_instance（基础设施参数归基础设施配置段）；**修正 Shutdown 顺序**：先排空 pushChan 再关闭 PushBus，确保 drain 阶段跨实例投递可用；**PushBus 接口**补充生命周期管理说明；**标注已知限制**：现有 UserMap.setOnline/setOffline 在 eventLoop 中的同步 Redis I/O；新增风险闸门条目 |
...
| 消息去重 | **SourceInstance 跳过 + sync.Map 兜底** | 广播模式下发送者实例通过 SourceInstance 字段直接跳过自投递（主防线），sync.Map 作为兜底防止跨实例重复（次防线） |
...
| 远端信封处理 | **异步 channel + worker pool** | PubSub handler 仅写入 remoteEnvChan，由 remoteEnvLoop workers 异步处理，避免慢投递阻塞 PubSub 接收管道 |
| 配置分层 | **基础设施参数归 cross_instance** | route_ttl_seconds、instance_alive_ttl_seconds 属于基础设施而非策略，放在 cross_instance 下可独立配置 |
...
pushChan → pushLoop → processPushTask()  [改造点]
    ...
    │
    └─ dispatchCrossInstance(task, msgData)
        │
...
远端接收 (异步):
    PubSub message → remoteEnvChan (buffered channel)
        ↓
    remoteEnvLoop workers → onRemoteEnvelope()
        ├─ SourceInstance == self? → 跳过 (v1.3 广播自投递防护)
        ├─ pushDedup 去重
        └─ 按 mode 投递本地连接
```
...
type PushEnvelope struct {
    ...
    SourceInstance string                `json:"source_instance"`            // 来源实例 ID（v1.3: 用于广播自投递防护）
...
**SourceInstance 字段**标记信封来源实例，接收端在广播模式下跳过来源实例的自投递（v1.3 修复）。
...
**心跳续期流量说明（v1.3 新增）：**
每次心跳（10s）通过 `RefreshAllConnRoutes` 批量续期本实例所有在线用户的路由 TTL。假设单实例 5,000 用户，Pipeline 批量执行 5,000 个 EXPIRE，单次 Pipeline 延迟约 1-2ms。20 个实例下 Redis 每 10s 承受约 100,000 次 EXPIRE，在 Redis 性能安全范围内（EXPIRE 是 O(1) 操作）。如未来连接规模超出预期，可考虑分批续期（每次心跳续期 1/N 用户并轮转）。
...
```go
// PushBus 抽象 Redis Pub/Sub 的发布与订阅。
//
// 生命周期管理（v1.3 补充）：
// - Subscribe* 方法在内部启动 goroutine 接收消息，通过 ctx 控制其生命周期。
//   ctx 被 cancel 后，订阅 goroutine 应自动退出。
// - Subscribe* 的 error 返回值仅表示订阅建立是否成功（channel 注册失败等），
//   不涵盖运行时的消息接收错误（运行时错误通过日志记录）。
// - Close() 关闭底层 Redis PubSub 连接，同时终止所有订阅 goroutine。
//   Close() 调用后，Publish* 方法将返回错误。
// - Close() 可安全重复调用（幂等）。
...
if 单聊 (sessionType == constant.SessionTypeSingle):
    return Route
...
> **v1.3 修正:** 移除 `localDelivered` 中间变量，`pushToLocalClients` 简化为无返回值，
> `dispatchCrossInstance` 移除未使用的 `localDelivered` 参数（过滤通过 instanceId 完成，不依赖已投递记录）。
...
func (s *WsServer) processPushTask(ctx context.Context, task *PushTask) {
    ...
    if s.cfg.WebSocket.CrossInstance.Enabled && s.pushBus != nil {
        s.dispatchCrossInstance(ctx, task, msgData)
    }
}
```
...
> **v1.3 修正:** 移除 `map[string][]ConnRef` 返回值。该返回值此前传入 `dispatchCrossInstance`
> 但从未被使用（远端过滤通过 `filterOutLocalInstance` 按 instanceId 完成）。
> 简化后函数签名与现有 `processPushTask` 的内联逻辑一致，降低认知负担。
    ...
    for _, userId := range task.TargetIds {
        ...
        for _, client := range clients {
            ...
            if err := client.PushMessage(ctx, msgData); err != nil {
                log.CtxDebug(ctx, "push to client failed: user_id=%s, conn_id=%s, error=%v",
                    userId, client.ConnId, err)
            }
        }
    }
}
...
> **v1.3 修正:** 移除未使用的 `localDelivered` 参数。

```go
func (s *WsServer) dispatchCrossInstance(ctx context.Context, task *PushTask, msgData *MessageData) {
...
> **v1.3 关键修复：广播自投递防护。**
> Redis Pub/Sub 的 broadcast channel 会将消息投递给所有订阅者，包括发布者自己。
> `pushToLocalClients` 已处理本实例用户，若 `onRemoteEnvelope` 不跳过自投递，
> 本实例用户会收到重复推送。通过 `SourceInstance == s.instanceId` 检查解决。
> Route 模式不受影响（`filterOutLocalInstance` 已排除本实例，route 信封不会发给自己）。

> **v1.3 新增：异步 channel 处理。**
> `onRemoteEnvelope` 不再由 PubSub goroutine 直接调用。PubSub handler 将 envelope
> 写入 `remoteEnvChan`（buffered channel），由 `remoteEnvLoop` worker pool 异步处理。
> 避免慢 client.PushMessage 阻塞 PubSub 接收管道导致后续消息积压。

```go
func (s *WsServer) onRemoteEnvelope(ctx context.Context, env *PushEnvelope) {
    // 1. 广播自投递防护：来源实例跳过（本实例已由 pushToLocalClients 处理）
    if env.Mode == PushModeBroadcast && env.SourceInstance == s.instanceId {
        return
    }

    // 2. 去重（本地 sync.Map，定期批量清理，作为兜底防线）
    if _, loaded := s.pushDedup.LoadOrStore(env.PushId, time.Now().UnixMilli()); loaded {
        return
    }

    // 3. 直接使用信封中的 MsgData（已是轻量 MessageData，无需再次转换）
    msgData := env.MsgData
...
// remoteEnvLoop 远端信封处理 worker（v1.3 新增）
// 从 remoteEnvChan 读取 PubSub 收到的信封，调用 onRemoteEnvelope 处理。
// 多个 worker 并发运行，通过 internalCtx 控制生命周期。
func (s *WsServer) remoteEnvLoop(ctx context.Context) {
    for {
        select {
        case <-ctx.Done():
            return
...
> **v1.3 已知限制:** 现有 `UserMap.Register` 内部调用 `setOnline`（`rdb.Set`），
> `UserMap.Unregister` 在用户完全离线时调用 `setOffline`（`rdb.Del`），这两个操作
> 是同步 Redis I/O，在 eventLoop 中执行。这是改造前的遗留问题，不在本期修复范围。
> 本期新增的 RouteStore 操作已全部异步化。后续优化方向：将 `setOnline`/`setOffline`
> 移至 WsServer 层异步执行，与 RouteStore 操作统一处理模式。
...
> **v1.3 修正:** `route_ttl_seconds` 和 `instance_alive_ttl_seconds` 从 `hybrid` 移至 `cross_instance`。
> 这两个参数属于跨实例基础设施（RouteStore 和 InstanceManager 使用），与混合策略选择无关，
> 放在 `cross_instance` 下可在 `hybrid.enabled=false` 时独立配置。
...
> **v1.3 修正:** TTL 字段从 `WebSocketHybridConfig` 移至 `WebSocketCrossInstanceConfig`。
...
> **v1.3 修正:** TTL 默认值跟随字段迁移至 cross_instance。
...
| remoteEnvChan 满 | 丢弃信封，记录告警日志，客户端通过 pull 补偿 |
...
> **v1.3 修正:** 去重改为双层防护——SourceInstance 跳过 + sync.Map 兜底。

广播模式下同一消息被所有实例收到。去重通过两层防护实现：
1. **主防线（SourceInstance 跳过）：** `onRemoteEnvelope` 在广播模式下检查 `env.SourceInstance == s.instanceId`，来源实例直接跳过。这避免了发送者实例通过广播重复投递本地用户。
2. **兜底防线（sync.Map 去重）：** 使用本地 `sync.Map`（key = `pushID`，value = 写入时间戳），由单个 goroutine 每秒定期批量清理超过 5s 的条目（`startDedupCleaner`）。防止网络异常导致的跨实例重复投递。

不依赖 Redis SETNX，减少一次网络往返；不为每条消息启动单独的清理 goroutine，避免高频推送时 goroutine 累积。
...
> **v1.3 新增:** WsServer 新增 `remoteEnvChan` 和 `remoteEnvLoop` worker pool，
> PubSub handler 仅写入 channel 不直接调用 `onRemoteEnvelope`，避免阻塞 PubSub 接收管道。
...
type WsServer struct {
    ...
    // v1.3: 远端信封异步处理
    remoteEnvChan chan *PushEnvelope  // buffered，容量复用 PushChannelSize
}
...
func (s *WsServer) Run() {
    ...
    // 跨实例组件（条件启动）
    if s.cfg.WebSocket.CrossInstance.Enabled && s.pushBus != nil {
        // v1.3: 启动远端信封处理 workers
        s.remoteEnvChan = make(chan *PushEnvelope, s.cfg.WebSocket.PushChannelSize)
        for i := 0; i < workerNum; i++ {
            go s.remoteEnvLoop(s.internalCtx)
        }

        // v1.3: PubSub handler 仅写入 channel，不直接处理
        envHandler := func(ctx context.Context, env *PushEnvelope) {
            select {
            case s.remoteEnvChan <- env:
            default:
                log.CtxWarn(ctx, "remote envelope channel full, push_id=%s dropped", env.PushId)
            }
        }
        s.pushBus.SubscribeInstance(s.internalCtx, s.instanceId, envHandler)
        s.pushBus.SubscribeBroadcast(s.internalCtx, envHandler)
...
> **v1.3 修正:** 调整 Shutdown 步骤顺序——先排空 pushChan（此时 PushBus 仍可发布，
> 跨实例投递正常），再关闭 PushBus，最后排空 remoteEnvChan 并 cancel 内部 ctx。
> 此前的顺序（先关 PushBus 再 drain）会导致 drain 阶段的 `dispatchCrossInstance`
> 调用 `PublishToInstance/Broadcast` 失败，跨实例消息丢失。

```go
func (s *WsServer) Shutdown(ctx context.Context) error {
    // 1. 停止接受新连接（由 HTTP server shutdown 保证）

    // 2. 停止 InstanceManager 心跳（路由表 TTL 后自动过期）
    if s.instMgr != nil {
        s.instMgr.Stop(ctx)
    }

    // 3. 排空 pushChan（PushBus 仍可用，跨实例投递正常）
...
pushDrained:
    ...
    // 4. 关闭 PushBus（停止接收和发送远端推送）
    if s.pushBus != nil {
        s.pushBus.Close()
    }

    // 5. 排空 remoteEnvChan（PushBus 已关闭，仅处理已入队的远端信封）
    if s.remoteEnvChan != nil {
        remoteCtx, remoteCancel := context.WithTimeout(ctx, 5*time.Second)
        defer remoteCancel()
        for {
            select {
            case env := <-s.remoteEnvChan:
                s.onRemoteEnvelope(remoteCtx, env)
            case <-remoteCtx.Done():
                goto remoteDrained
            default:
                goto remoteDrained
...
remoteDrained:
    ...
    // 6. cancel 内部 ctx，停止所有内部 goroutine（eventLoop、pushLoop、remoteEnvLoop、dedupCleaner）
    if s.internalCancel != nil {
        s.internalCancel()
    }

    // 7. 关闭所有客户端连接（触发 UnregisterConn 异步清理路由）
...
9. 广播模式下发送者实例不能重复投递本地用户（SourceInstance 跳过检查）（v1.3 新增）
10. PubSub handler 不能同步调用 onRemoteEnvelope，必须通过 channel 异步处理（v1.3 新增）
11. Shutdown 中 pushChan 排空必须在 PushBus.Close() 之前，确保 drain 阶段跨实例投递可用（v1.3 新增）
...
| 限制项 | 说明 | 影响 | 后续方向 |
|--------|------|------|----------|
| eventLoop 中 setOnline/setOffline 同步 Redis | 现有 `UserMap.Register` 调用 `setOnline`（`rdb.Set`）、`Unregister` 调用 `setOffline`（`rdb.Del`），在 eventLoop 单 goroutine 中同步执行 | Redis 慢响应时阻塞所有 register/unregister 处理 | 将 setOnline/setOffline 移至 WsServer 层异步执行 |
| 心跳续期全量 EXPIRE | 每次心跳（10s）对所有在线用户路由做 Pipeline EXPIRE | 20 实例 × 5000 用户 = 100,000 次 EXPIRE/10s，Redis 可承受但非最优 | 分批续期或加大 route TTL 减少频率 |
...
| 文件 | 改动内容 |
|------|---------|
| `internal/config/config.go` | 新增 `WebSocketCrossInstanceConfig`（含 TTL 字段）、`WebSocketHybridConfig`（纯策略字段） |
| `config/config.yaml` | 新增 `websocket.cross_instance`（含 TTL）和 `websocket.hybrid`（纯策略）配置段 |
| `pkg/constant/constant.go` | 新增 4 个 Redis Key/Channel 模式 |
| `internal/gateway/ws_server.go` | 新增字段（routeStore/pushBus/selector/instanceId/pushDedup/internalCtx/internalCancel/instMgr/remoteEnvChan），拆分 `processPushTask` 为三方法，新增 `remoteEnvLoop`，`registerClient`/`unregisterClient` 新增异步 RouteStore 调用，`GetUsersOnlineStatus` 合并本地+远端，`Run` 改为无参数使用内部 ctx，新增 `Shutdown` 方法 |
...
**WsServer (onRemoteEnvelope — v1.3 新增):**
- 广播模式 + SourceInstance == self → 跳过，不投递（自投递防护）
- 广播模式 + SourceInstance != self → 正常按 TargetUserIds 过滤本地用户投递
- route 模式 + SourceInstance == self → 正常处理（route 不需要 SourceInstance 防护，但应无害）
- 重复 PushId → pushDedup 拦截，不重复投递
- remoteEnvChan 满 → 信封被丢弃，记录告警日志
...
**WsServer (lifecycle):**
- `Run()` 后所有 goroutine 启动（eventLoop、pushLoop、remoteEnvLoop、dedupCleaner）
- `Shutdown()` 后所有 goroutine 停止（internalCtx cancelled）
- `Shutdown()` 先排空 pushChan（跨实例投递正常），再关闭 PushBus，再排空 remoteEnvChan
...
- **验证发送者实例上的本地用户不会收到重复推送（v1.3）**

**广播自投递防护（v1.3 新增）:**
- 用户 1、2 都连接实例 A，用户 3 连接实例 B
- 用户 1 发群聊消息（触发 broadcast）
- 验证用户 2 在实例 A 上只收到 1 次推送（不因广播回传而重复）
- 验证用户 3 在实例 B 上收到 1 次推送
...
5. **eventLoop Redis I/O 异步化** — 将现有 UserMap.setOnline/setOffline 移至 WsServer 层异步执行（v1.3 标注的已知限制）
6. **心跳续期分批优化** — 按用户分组轮转续期，降低单次心跳的 Redis EXPIRE 数量（v1.3 标注的已知限制）
